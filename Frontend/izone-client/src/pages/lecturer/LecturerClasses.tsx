...
const LecturerClasses: React.FC = () => {
  ...
  const fetchClasses = async () => {
    try {
      ...
      // Get user info to filter classes taught by current lecturer
      ...
      console.log('🔄 Đang gọi API phân trang cho giảng viên:', {
      ...
      const response = await lopHocService.getPaginatedByLecturer(
        user.giangVienID,
            ...
            <div key={classItem.lopID} className="class-card" style={{
              ...
              <div className="class-footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div className="action-buttons">
                  <button
                    className="btn-view"
                    onClick={() => navigate(`/lecturer/class/${classItem.lopID}`)}
                    style={{
                      padding: '8px 16px',
